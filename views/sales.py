...
class SalesView(ttk.Frame):
    def __init__(self, root, controller):
        ...
        self.cart_listbox.grid(row=1, column = 0, columnspan=2, rowspan=3, padx=10, pady=5)

        self.label_total = tk.Label(self, text="Total(NRs):")
        self.label_total.grid(row=1, column=2, padx=10, pady=5)
        
        self.text_total = tk.Label(self, text='0.0')
        self.text_total.grid(row=1, column=3, padx=10, pady=5)

        self.label_buyer = tk.Label(self, text="Buyer's Name:")
        self.label_buyer.grid(row=2, column=2, padx=10, pady=5)

        self.entry_buyer = tk.Entry(self)
        self.entry_buyer.grid(row=2, column=3, padx=10, pady=5)

        self.label_paid = tk.Label(self, text="Paid Amount(NRs):")
        self.label_paid.grid(row=3, column=2, padx=10, pady=5)
    ...
    def show_checkout_details(self, summary):
        messagebox.showinfo("Checkout", f"Total: Rs{summary[0]:.2f}\nPaid: Rs{summary[1]:.2f}\nReturn: Rs{summary[2]:.2f}\nRemaining: Rs{summary[3]:.2f}")
